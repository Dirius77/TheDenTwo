...
public sealed partial class ChatSystem : SharedChatSystem
{
        ...
        )
    {
        ...
        // DEN - Pass Language to speaking related functions.
    ...
    private void SendEntitySpeak(
        ...
        ChatTransmitRange range,
        string? nameOverride,
        ComplexChatMessage chatMessage, // DEN
        bool hideLog = false,
        bool ignoreActionBlocker = false
        )
    {
        ...
        {
            var nameEv = new TransformSpeakerNameEvent(source, Name(source), chatMessage.Language); // DEN Languages
        ...
        )
    {
        ...
        foreach (var (session, data) in GetRecipients(source, WhisperMuffledRange))
        {
            ...
            // DEN Language Start
            var understandingEvent = new DetermineUnderstandingEvent(source, chatMessage.Language, message);
        ...
        var ev = new EntitySpokeEvent(source, message, channel, obfuscatedMessage, chatMessage.Language); // DEN Languages
        RaiseLocalEvent(source, ev, true);
        if (!hideLog)
            if (originalMessage == message)
            {
    ...
    private void SendInVoiceRange(ChatChannel channel, string message, string wrappedMessage, string? langObfuscatedMessage, string? wrappedLangObfuscatedMessage, EntityUid source, ChatTransmitRange range, NetUserId? author = null, LanguagePrototype? language = null) // DEN
    {
        foreach (var (session, data) in GetRecipients(source, VoiceRange))
        {
            ...
            // DEN Send Language based on understanding.
            if (language is not null && session.AttachedEntity is not null)
            {
                // Error checking, this should never happen and we're praying the compiler is smart enough to lift
                // this check outside the loop because it won't let me do it manually.
                if (langObfuscatedMessage is null || wrappedLangObfuscatedMessage is null)
                {
                    _sawmill.Warning("Tried to send a language based message without obfuscation, this shouldn't happen.");
                    return;
                }
                ...
                if(understandingEvent.Understands)
                    ...
                    _chatManager.ChatMessageToOne(channel, langObfuscatedMessage, wrappedLangObfuscatedMessage, source, entHideChat, session.Channel, author: author);
            }
            else
            {
                _chatManager.ChatMessageToOne(channel, message, wrappedMessage, source, entHideChat, session.Channel, author: author);
    ...
    public string TransformSpeech(EntityUid sender, string message, LanguagePrototype language) // DEN Pass Language
    {
        var ev = new TransformSpeechEvent(sender, message, language);
